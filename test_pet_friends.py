...
def test_add_new_pet_with_valid_data(name='Patrik', animal_type='star', age='2', pet_photo='images/Patrik.jpeg'):
...
def test_add_new_pet_with_valid_data2(name='Patrik', animal_type='star', age='2', pet_photo='images/Patrik.jpeg'):
...
def test_add_new_pet_with_valid_data3(name='Patrik', animal_type='star', age='25', pet_photo='images/Patrik.jpeg'):
...
def test_add_new_pet_with_valid_data4(name='Patrik', animal_type='star', age='2', pet_photo='images/Patrik.jpeg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, name='dog', animal_type='dwordog', age='5', pet_photo='images/dog.jpeg')
        # pf.post_new_friends(auth_key, name='Snuppy', animal_type='dog', age='5', pet_photo='images/korgi.jpeg')
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_delete_self_pet2(name='Patrik', animal_type='star', age='2'):
    ...
    # Берём id первого питомца из списка и отправляем запрос на удаление
    pet_id = my_pets['pets'][0]['id']
    status, _ = pf.delete_pet(auth_key,  name)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    assert status == 200
    assert name not in pet_id


def test_successful_delete_self_pet3(name='Patrik', animal_type='star', age='2'):
    """Проверяем возможность удаления type питомца"""
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    pet_id = my_pets['pets'][0]['id']
    status, _ = pf.delete_pet(auth_key,  animal_type)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    assert status == 200
...
def test_put_self_pet_info_3(name='Patrik', animal_type='star', age='100'):
